# Görüşme ve Yazışma Notları

## 2026-04-27 — WhatsApp yazışması (ESMARK ↔ Fahrettin Durak)

**Konu:** Piyasa ve mevcut durum analizi onayı, rakip listesi.

### ESMARK
> Merhaba Fahrettin bey, tekrar memnun oldum diyerek söze girmiş olayım.
>
> Piyasa ve mevcut durum analizi için aşağıdaki araştırma bilgilerine onayınızı rica ederiz. Bu firmalara doğrudan "Rakip Firmalar" diyebilir miyiz? Eklemek veya düzeltmek istedikleriniz varsa iletebilir misiniz?

### Önerilen rakip — Mekasis Otomasyon
- Web: https://mekasisotomasyon.com/
- Bursa'da eksantrik pres tamiri, revizyonu ve hat taşıma konusunda en aktif rakiplerinden.
- Kendi bünyelerinde 5 eksen işleme yetenekleri olduğunu belirtiyorlar — teknik kapasite açısından öne çıkıyorlar.
- ESMARK gözlemi: "Birçoğu revizyon sırasında ihtiyaç duyulan büyük parça işlemelerini (volan, krank mili vb.) dışarıda yaptırıyor olabilir."

### Fahrettin Bey'in cevapları
> Bizde de talaşlı imalat makineleri mevcut, hatta piyasada 4 ila 5 adet olan **çap 780 boy 5000 mm tornamız** var. Görüştüğümüzde bunların detaylarını verebiliriz.

> Evet rakip firmalar. Fakat hepsinde bahsedilen makineler yok dışarıdan, destek alıyorlar ama var gibi gösteriyorlar. **Biz bunları sitede resim, video, görseller ile kanıtlayabiliriz.**

> [Dışarı iş veriyor olabilirler yorumuna onay]: Doğrudur. **Hatta ana işlemleri bile.**

### Karar
- Mekasis Otomasyon rakip olarak onaylandı
- Ana farklılaştırıcı netleşti: tüm işler bünyede yapılıyor, görsel/video ile kanıtlanacak
- Yarın (2026-04-28) telefon görüşmesi → toplantı planlanacak

---

## 2026-04-28 — Telefon görüşmesi (Fahrettin Durak, ~75 dk, 12:15)

**Format:** Telefon görüşmesi, ~75 dakika. Görüşme sonrası ESMARK toplantı notlarını WhatsApp üzerinden müşteriye iletti.

### ESMARK kapanış mesajı (WhatsApp)
> Merhaba Fahrettin bey,
> Öncelikle sizinle tanışmış olduğum için çok memnunum. Gayet verimli ve yerinde bir toplantı olduğuna inanıyorum. Sorularınıza ve aklınızdaki bazı netleşmesi gereken detaylara cevap verebildiysem ne mutlu.
>
> Toplantı sonunda da ifade ettiğim gibi; dijitalin bu kadar hayatın bir parçası olduğu günümüz dünyasında, her 1 günlük gecikme ileride yaşanması olası süreçsel zorlukları önümüze koyabilmektedir. Bu bağlamda, ne kadar bu dijital dünyayı kabullenmekte ben bile güçlük çeksem de sizleri aramızda görmek ve birlikte dijital dünyada imzanızı atmak isterim.

### Mevcut durum analizi (sunulan tespitler)
- Web sitesi 2020'den beri güncellenmemiş, alt sayfalar açılmıyor
- Instagram aktif değil (3-4 fotoğraf)
- Google Business eski adresi gösteriyor, yeni lokasyon bulunamıyor
- E-posta adresi web sitesinde yok
- Sosyal medya entegrasyonu yok, foto/video arşivi yok

### Şirket avantajları (toplantıda doğrulanan)
- **Taşeronsuz üretim — %60-70 kendi makine parkında**
- **5 metre torna** — Bursa bölgesinde 4-5 firmada var *(not: 2026-04-27 yazışmasında müşteri "çap 780 mm × boy 5000 mm" demişti; "5 metre" büyük olasılıkla **boy** ölçüsü — sözleşme öncesi yazılı doğrulama alınacak)*
- Eksantrik pres, hidrolik pres, talaşlı imalat makineleri
- Mekanik + hidrolik + otomasyon — üçünü birden yapabilme
- Büyük parça işleme kapasitesi
- **Sürekli çalışılan referanslar:** Arçelik, Airpol, Betek Metal

### Sunulan dijital paket
- Yeni web sitesi tasarımı ve yazılımı
- Instagram, Facebook, LinkedIn hesap kurulumu ve yönetimi
- Google Business güncellemesi ve optimizasyonu
- SEO + GEO (yapay zeka destekli arama optimizasyonu)
- Sosyal medya içerik yönetimi
- Müşteri iletişim sistemi (CRM entegrasyonu)

### Reklam ve pazarlama stratejisi
- Google Ads (uzman tarafından bizzat yönetilecek)
- Meta Ads (Instagram + Facebook)
- LinkedIn reklamları
- Yapay zeka destekli içerik üretimi
- Müşteri geri bildirimlerinden kelime analizi
- Before/after proje gösterimleri

### Fiyatlandırma (sözel olarak iletildi)
- **Web sitesi ve kurulum:** 20.000 TL (normal 22.500 TL — indirim uygulanıyor)
- **Aylık hizmet bedeli:** 20.000 TL
- **Reklam bütçesi:** ayrı kalem — değişken, müşteri ile birlikte süreç içinde belirlenecek
- **Ödeme:** web sitesi taksitlendirilebilir (3-4 taksit)
- **Başlangıç:** iş başlangıcında ücret alınıyor

### Süreç ve zaman çizelgesi
- WhatsApp grubu kurulacak — süreç takibi
- Google Drive'da ortak klasör
- Görüşmelerden ~1 hafta sonra başlangıç
- 2-3 hafta içinde tüm sistemler hazır
- Foto/video çekimleri planlanacak
- Mevcut stok arşivlerinden görsel ve video alınacak
- Makine envanteri netleştirilecek

### Sonraki adımlar (operasyonel)
- Reklam bütçesi için maliyet hesaplaması yapılacak
- Web sitesi domain kontrolü ve site altyapısı
- Instagram hesap erişim bilgileri ve Meta kurulumları
- Google Business hesap devri ve yönetimi

### Sonuç
**Karar süreci:** Müşteri düşünme zamanı istedi. Onay sonrası başlangıç 1 hafta içinde, 2-3 haftada teslim hedefi.

### Toplantı sonrası iç değerlendirme
Görüşme transkripti dış bir değerlendiriciye okutuldu. Çıkan sonuç: müşterinin 6 farklı şekilde tekrarladığı *"bu büyüklükte bir şeye hazır değilim"* sinyali iltifat sanılmış; aylık 20K paket ve uzun vizyon anlatısı müşterinin "şu an bana fazla" yargısını oturtmuş. Detaylı analiz ve gelecek toplantılar için ders defteri: [post-mortem-2026-04-28.md](./post-mortem-2026-04-28.md).

**Strateji revizyonu:** Aylık bağ ve reklam yönetimi paketten çıkarılıyor; tek seferlik düşük taahhütlü kurulum paketi ile dönülecek. Detay: [teklif-v2.md](./teklif-v2.md).

---

## 2026-05-04 — Telefon dönüşü (Fahrettin Durak)

**Format:** Telefon. ESMARK'ın geçen hafta boyunca yaptığı birkaç aramaya müşteri bugün dönüş yaptı.
**Sonuç:** Müşteri yarın (2026-05-05 Salı 14:00) ESMARK ofisine ziyarete geliyor — yüz yüze görüşme.

**Anlam:**
- Planlanan WhatsApp takip mesajı (aşağıda) **devre dışı** — müşteri kendi döndü, ofis ziyareti talep etti.
- Müşteri yüz yüze görüşmeye geliyorsa **ilgi var** sinyali; "şu an düşünmüyorum" senaryosu elendi.
- Pozisyon: post-mortem ders defterindeki **danışman tutumu** — önce qualifying soruları, sonra çözüm. Sunum + plan dayatma.

---

## 2026-05-05 — Ofis ziyareti **İPTAL**

**Durum:** Toplantı yapılmadı. Fahrettin Bey randevudan ~30 dk önce (≈13:30) aradı, işi çıktığını bildirdi.
**Müşteri tarafı sinyalleri:**
- Saat sorunu var — gün içi 14:00 ona zor geliyor olabilir (üst üste iki kez randevu verirken zorluk: önce dönüş gecikmesi, şimdi son dakika iptal).
- Alternatif önerileri: **akşam online görüşme** veya **gün içi başka bir vakit**.
- "Haberleşeceğiz, 2 gün içinde tekrar bağlantı kurarız" dedi → karar verme niyeti devam ediyor, tamamen geri çekilmedi.

**ESMARK pozisyonu (öneri):**
- Top yine müşteride; baskı yok. 2 gün içinde dönmezse 2026-05-07 Perşembe sabahı (10-11) kısa WhatsApp ile yeniden değme — **sadece müsait saat sorulur**, içerik konuşulmaz.
- Müşteri akşam online tercih ederse Google Meet linki + Calendar daveti hazırla; ofise zorlamayı bırak.
- Yüz yüze yerine online'a geçmek toplantının niteliğini değiştirmez ama müşterinin yükünü azaltır → **bu projede online lehine esne**.

**Calendar:** Etkinlik silindi, hatırlatmalar iptal.

### Hazırlık (yeni randevu için yine geçerli)
- [ ] [teklif-v2.md](./teklif-v2.md) son hali — 15.000 TL tek seferlik, web + GBP + temel SEO, aylık bağ yok
- [ ] [post-mortem-2026-04-28.md](./post-mortem-2026-04-28.md) → 8 qualifying sorusu yeniden gözden geçirilsin
- [ ] Açık doğrulamalar listesi yanında (defter/yazılı):
  - 5 m torna boy mu çap mı (27.04 yazışmasındaki "çap 780 × boy 5000 mm" tutarlılığı)
  - Yeni adres tam metni (GBP için)
  - Tam makine envanteri yazılı liste
  - Referans müşteri (Arçelik, Airpol, Betek Metal) logo + isim kullanma izni
  - Domain/hosting sahipliği bilgisi
  - Foto/video arşivi durumu

### Kırmızı çizgiler (post-mortem'den hatırlatma)
- "Sizin sisteme ihtiyacınız var, web sayfasına değil" gibi büyük cümleler **bu toplantıda yok** — müşteri kendi alanında uzman.
- Aylık 20K paket ve "vizyon önce, fiyat sonra" sırası **tekrarlanmaz** — düşük taahhüt + net kapsam + net fiyat.
- Çapraz referans (Akademi Döner, Bambini, Amerika fiyatları) **getirme** — ölçek kaygısını derinleştirdi.
- Müşteri "düşüneceğim" derse zorlama; toplantıyı kapat, yazılı özet WhatsApp'tan iletilir.

### Görüşme sonrası
- Toplantı notları bu dosyaya, yeni bir bölüm olarak.
- Karar netleşirse: domain/hosting devri, erişim bilgileri, foto/video planlama → ayrı bölüm.

### Hazırlık checklist
- [ ] [teklif-v2.md](./teklif-v2.md) son hali — 15.000 TL tek seferlik, web + GBP + temel SEO, aylık bağ yok
- [ ] [post-mortem-2026-04-28.md](./post-mortem-2026-04-28.md) → 8 qualifying sorusu yeniden gözden geçirilsin
- [ ] Açık doğrulamalar listesi yanında (defter/yazılı):
  - 5 m torna boy mu çap mı (27.04 yazışmasındaki "çap 780 × boy 5000 mm" tutarlılığı)
  - Yeni adres tam metni (GBP için)
  - Tam makine envanteri yazılı liste
  - Referans müşteri (Arçelik, Airpol, Betek Metal) logo + isim kullanma izni
  - Domain/hosting sahipliği bilgisi
  - Foto/video arşivi durumu

### Kırmızı çizgiler (post-mortem'den hatırlatma)
- "Sizin sisteme ihtiyacınız var, web sayfasına değil" gibi büyük cümleler **bu toplantıda yok** — müşteri kendi alanında uzman.
- Aylık 20K paket ve "vizyon önce, fiyat sonra" sırası **tekrarlanmaz** — düşük taahhüt + net kapsam + net fiyat.
- Çapraz referans (Akademi Döner, Bambini, Amerika fiyatları) **getirme** — ölçek kaygısını derinleştirdi.
- Müşteri "düşüneceğim" derse zorlama; toplantıyı kapat, yazılı özet WhatsApp'tan iletilir.

### Görüşme sonrası
- Toplantı notları bu dosyaya, yeni bir bölüm olarak.
- Karar netleşirse: domain/hosting devri, erişim bilgileri, foto/video planlama → ayrı bölüm.

---

## 2026-05-05 / 2026-05-06 — Planlanan takip iletişimi (taslak — DEVRE DIŞI)

**Durum:** **Uygulanmadı.** Müşteri 2026-05-04'te kendi dönüş yaptı ve ofis ziyareti talep etti — bu mesaja gerek kalmadı. Aşağıdaki taslak ileride benzer "soğuk takip" senaryoları için referans olarak saklanıyor.

### Neden bu pencere
Toplantı 2026-04-28 Salı yapıldı. Tatil takvimi nedeniyle önündeki günler sürekli atılır:
- 04-29 Çar / 04-30 Per — çok erken, "düşünmedim bile" yanıtı gelir
- **05-01 Cuma — 1 Mayıs İşçi Bayramı (resmi tatil)**
- 05-02 Cmt / 05-03 Pzr — hafta sonu, iş mesajı zayıflık sinyali
- 05-04 Pzt — Cuma + hafta sonu uzantısı (köprü günü), çoğu firma yarım veya kapalı
- **05-05 Salı / 05-06 Çar — iş haftası tam oturmuş, toplantıdan 7-8 gün sonra → ideal**

Bu pencerede sabah saatleri (10-11) tercih edilir; mesai başında, öğle yoğunluğu öncesi.

### Mesaj taslağı
> Merhaba Fahrettin Bey, görüşmemiz üzerinde tekrar düşündüm.
>
> Söyledikleriniz haklıydı — anlattığım yapı sizin şu anki ihtiyacınız için fazla geniş kalıyordu. Bunun üzerine size daha sade, aylık bağlantı içermeyen bir kurulum paketi hazırladım.
>
> İlgilenirseniz detayları her zaman konuşabiliriz. İyi haftalar dilerim.

### Mesajın taşıdığı pozisyon
- **Müşteriyi haklı çıkarıyor** — "söyledikleriniz haklıydı". Geri çekilen müşteri için yüz kurtarıcı.
- **Aylık bağ olmadığını net söylüyor** — temel itirazı doğrudan adresliyor.
- **Baskı yok** — "ilgilenirseniz", soru işareti yok, "ne dersiniz" yok. Müşteri kendi gelir.
- **Detay yok** — fiyat, süre, paket içeriği mesajda geçmiyor. Müşteri yanıt verirse o zaman [teklif-v2.md](./teklif-v2.md) içeriği iletilir.

### Mesaj sonrası senaryolar
| Müşteri yanıtı | Aksiyon |
| -------------- | ------- |
| "Detay nedir?" | teklif-v2 paketini özetle ilet (15.000 TL tek seferlik, web + GBP + temel SEO, aylık bağ yok). Görüşme öner. |
| "Şu an düşünmüyorum" | "Anlıyorum, ileride aklınıza geldiğinde her zaman buradayım" — kapıyı açık bırak. 2-3 ay sonra tekrar light dokunuş. |
| Yanıt yok (1 hafta) | Tek bir sessiz takip: kısa görsel/değer paylaşımı (örn. "Mekanik bakım sektöründe yapay zeka aramalarda öne çıkma örneği" gibi içerik). Üçüncü mesaj yok. |
